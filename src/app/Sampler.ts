...
export type MinMaxSampleResult = {
    min: Float32Array;
    max: Float32Array;
}
export function minMaxSample(arr: Float32Array, samples: number): MinMaxSampleResult {
    const groupSize = getGroupSize(arr.length, samples);
    const actualSamples = Math.floor(arr.length / groupSize);
    const outputArrayMin = new Float32Array(actualSamples);
    const outputArrayMax = new Float32Array(actualSamples);
    ...
    for (let index = 0; index < actualSamples; index++) {
        ...
        const min = batch.reduce((min, v) => Math.min(min, v), Infinity);
        const max = batch.reduce((max, v) => Math.max(max, v), -Infinity);
        outputArrayMin[index] = min;
        outputArrayMax[index] = max;
    }

    return { min: outputArrayMin, max: outputArrayMax };
}